...
## Challenge: #TidyTuesday 2024 week 31
## Data:      Summer Movies
## Author:    Steven Ponce
## Date:      2024-08-01
...
tt <- tidytuesdayR::tt_load(
  ...
  week = base::as.double("31")
)

summer_movie_genres <- tt$summer_movie_genres |> clean_names() |> glimpse()
summer_movies <- tt$summer_movies |> clean_names() |> glimpse()
...
glimpse(summer_movie)
skim(summer_movies)


## 4. TIDYDATA ----
data_plot <- summer_movies |> 
    ...
    mutate(
        ...
        genre_count = fct_reorder(genre_count, average_rating, .desc = FALSE),
        highlight = ifelse(average_rating >= 6.29, "yes", "no")
        ) 
...
caption_col  <- "#3D3D3D"
text_col     <- "#3D3D3D"
col_palette  <- MetBrewer::met.brewer("Peru1")[c(2,3)] 

### |-  titles and caption ----
# icons
tt <- str_glue("#TidyTuesday: { 2024 } Week { 31 } &bull; Source: IMDb non-commercial datasets<br>")
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Cinematic Scores: Analyzing IMDb Ratings by Genre Across Summer Releases")

subtitle_text <- str_glue("Exploring the highs and lows of summer movie ratings across various film genres.")
...
### |-  final plot ----  
data_plot |>
    ggplot(aes(x = average_rating, y = genre_count, fill = highlight)) +
    ...
    geom_textbox(
        aes(
            label = scales::number(average_rating, accuracy = 0.01),
            hjust = ifelse(average_rating > 6.28, -0.02, .2),
        ),
        fill = NA,
        box.colour = NA,
        size = 2.8,
    ...
    geom_vline(
        xintercept = mean(data_plot $average_rating),
        linewidth = .2,
        color = 'gray'
    ) +
...
# P distributional   0.4.0    2024-02-07 [?] CRAN (R 4.4.0)
