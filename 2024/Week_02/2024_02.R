...
## Challenge: #TidyTuesday 2024 week 02
## Data:      Canadian NHL Player Birth Dates
## Author:    Steven Ponce
## Date:      2024-01-09
...
tt <- tidytuesdayR::tt_load(x = base::as.double("2024"), 
                            week = base::as.double("02")) 

canada_births_1991_2022 <- tt$canada_births_1991_2022 |> clean_names() |> glimpse()
nhl_player_birth        <- tt$nhl_player_births |> clean_names() |> glimpse()
...
skim(nhl_player_birth)
skim(canada_births_1991_2022)
...
# Canada births
canada_births_1991_2005 <- canada_births_1991_2022 |> 
    
    # filter years 1991-2005
    filter(year >= 1991 & year <= 2005) |> 
    
    # get months abbreviations and quarter
    mutate(
        month_abb   = month(month,label = TRUE, abbr = TRUE),
    ...
    # get births per year
    group_by(year) |>
    mutate(births_per_year = sum(births)) |> 
    
    # get births per quarter
    group_by(year, quarter, births_per_year) |> 
    summarise(births_per_quarter = sum(births)) |> 
    ungroup()|>
    
    # get births percentage
    mutate(
        births_pct_quarter = births_per_quarter / births_per_year,
        source = "canada") |> 
    distinct()


# NHL Canadian player's births
nhl_players_1991_2005 <- nhl_player_birth |> 
    ...
    # get months abbreviations and quarter
    mutate(
        month_abb = month(birth_month,label = TRUE, abbr = TRUE),
        quarter_num = quarter(birth_month),
        quarter     = case_when(
            quarter_num == "1" ~ "Q1",
            quarter_num == "2" ~ "Q2",
            quarter_num == "3" ~ "Q3",
    ...
    # get births per year
    group_by(birth_year) |>
    mutate(births_per_year = n()) |>
    
    # get births per quarter
    group_by(birth_year, quarter) |>
    mutate(births_per_quarter = n()) |>
    ungroup() |> 
    ...
    select(birth_year, quarter, births_per_year, births_per_quarter, births_pct_quarter, source) |> 
...
data_plot <- bind_rows(canada_births_1991_2005, nhl_players_1991_2005) |> 
    mutate(source = fct_reorder(source, births_pct_quarter))
...
col_palette  <- c("canada" = "#E65E57", "nhl" = "#E6CC57")


### |-  titles and caption ----
tt <- str_glue("#TidyTuesday: 2024 Week 02 &bull; Source: Canadian NHL Player Birth Dates <br>")  
X  <- str_glue("<span style='font-family:fa6-brands'>&#xe61b;</span>")   
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

title_text    <- str_glue("Are Canadian HNL players likely born in the first\nquarter of the year?") 

canada <- str_glue("<span style='color:{ col_palette['canada'] }'>**Canadians**</span>")
nhl    <- str_glue("<span style='color:{ col_palette['nhl'] }'>**NHL Players**</span>")
...
### |-  final plot ----  
data_plot |> 
    ggplot(aes(x = quarter, y = births_pct_quarter, fill = source)) +
    
    # Geoms
    geom_point(size = 3, alpha = 0.25, shape = 21, col   = text_col,
               position = position_jitterdodge(seed = 2024, 
                                               jitter.width = .2)) +
...
# P R.oo           1.25.0  2022-06-12 [?] CRAN (R 4.3.1)
