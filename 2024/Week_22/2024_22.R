...
## Challenge: #TidyTuesday 2024 week 22
...
tt <- tidytuesdayR::tt_load(x = base::as.double("2024"),
                            week = base::as.double("22"))

planting_2020 <- tt$planting_2020 |> clean_names() |> glimpse()
planting_2021 <- tt$planting_2021 |> clean_names() |> glimpse()
harvest_2020  <- tt$harvest_2020 |> clean_names() |> glimpse()
harvest_2021  <- tt$harvest_2021 |> clean_names() |> glimpse()
spending_2020 <- tt$spending_2020 |> clean_names() |> glimpse()
spending_2021 <- tt$spending_2021 |> clean_names() |> glimpse()
...
planting <- combine_years(planting_2020, planting_2021) |> glimpse()
harvest  <- combine_years(harvest_2020, harvest_2021) |> glimpse()
spending <- combine_years(spending_2020, spending_2021) |> glimpse()
...
### |- plot data ---  
harvest_summary_veg <- harvest |>
  summarize(
    total_weight = sum(weight, na.rm = TRUE),                                   # total weight harvested by vegetable and year
    .by = c(vegetable, year)
  ) |>
  group_by(vegetable) |>
  mutate(
    yoy_change = (total_weight - lag(total_weight)) / lag(total_weight) * 100   # YoY percentage change
  ) |>
  filter(!is.na(yoy_change)) |>
  ungroup() |>
  mutate(
    vegetable = str_to_title(vegetable),
    vegetable = fct_reorder(vegetable, yoy_change),
...
caption_col  <- "#d7d7d8"   
text_col     <- "#d7d7d8"  
col_palette  <- c("increase" = "#4CAF50", "decrease" = "#F44336")
...
# text
title_text    <- str_glue("Lisa\'s Vegetable Garden") 

subtitle_text <- str_glue("Year-over-Year Percentage Change in Total Weight Harvested by Vegetable")
...
### |-  Plot  ---- 
harvest_summary_veg |>
    ggplot(aes(x = vegetable, y = yoy_change, color = direction)) +
    ...
    geom_segment(
        aes(x = vegetable, xend = vegetable, y = 0, yend = yoy_change),
        size = 1.2
    ) +
    geom_point(
    ...
    geom_text(
        ...
        hjust = ifelse(harvest_summary_veg$yoy_change > 0, -0.55, 1.5),
