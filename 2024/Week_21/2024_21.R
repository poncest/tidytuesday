...
## Challenge: #TidyTuesday 2024 week 21
## Data:      Carbon Majors emissions data
## Author:    Steven Ponce
## Date:      2024-05-21
...
tt <- tidytuesdayR::tt_load(x = base::as.double("2024"),
                            week = base::as.double("21"))

emissions <- tt$emissions |> clean_names() |> glimpse()
...
emissions |> count(parent_entity, sort = T)
emissions |> count(parent_type, sort = T)
emissions |> count(commodity, sort = T)
...
# China (Coal) Subset
china_coal <- emissions |>
    filter(parent_entity == "China (Coal)")

# Calculate total emissions by commodity
total_emissions_by_commodity <- china_coal |>
    group_by(commodity) |>
    summarize(total_emissions = sum(total_emissions_mt_co2e, na.rm = TRUE)) |>
    arrange(desc(total_emissions))

# Reorder commodity factor levels based on total emissions
china_coal <- china_coal |>
    mutate(commodity = factor(commodity, levels = total_emissions_by_commodity$commodity))  

# Bar Labels for China (Coal) Subset
china_coal_labels <- china_coal |>
...
# 5. VISUALIZATION ---- 
### |- plot aesthetics ---- 
bkg_col      <- colorspace::lighten("#E4D6A7", .9)  
title_col    <- "gray10"             
subtitle_col <- "gray10"     
caption_col  <- "gray20"   
text_col     <- "gray20"    
...
### |-  titles and caption ----
# icons
tt <- str_glue("#TidyTuesday: { 2024 } Week { 21 } &bull; Source: CarbonMajors dataset<br>")  
...
### |-  Emissions by Parent Entity plot ---- 
col_palette  <- c("yes" = "#9B2915", "no" = "#E9B44C")

### |-  Plot 1: Top 10 Parent Entities by Total Emissions ---- 
...
### |-  Plot 2: Faceted Line Chart by Commodity ---- 
col_palette_2  <- c("#9B2915", "#E9B44C", "#1C110A", "#50A2A7")

p2 <- china_coal |>
  ggplot(aes(x = year, y = total_emissions_mt_co2e, 
    fill = commodity, color = commodity)
    ) +
  # Geoms
  geom_area(alpha = 0.8) +
  geom_point(data = china_coal |> filter(year == 1945), size = 2) +
  geom_point(data = china_coal |> filter(year == 2022), size = 2) +
  geom_text(
    data = china_coal_labels |> filter(year == 1945),
    aes(x = year, y = total_emissions + (total_emissions * 100), label = bar_label),
    hjust = 0.5,
    nudge_x = 0.005,
    vjust = 0,
    fontface = "bold",
  ...
  geom_text(
    data = china_coal_labels |> filter(year == 2022),
    aes(x = year, y = total_emissions + (total_emissions * .25), label = bar_label),
    hjust = 0.85,
    nudge_x = -0.005,
    vjust = 1,
    fontface = "bold",
  ...
  # Scales
  scale_x_continuous(
      breaks = seq(1945, 2022, by = 25),
      limits = c(1945, 2022),
      expand = c(0, 1),
  ) +
  scale_y_continuous(labels = comma_format()) +
  scale_fill_manual(values = col_palette_2) +
  scale_color_manual(values = col_palette_2) +
  coord_cartesian(clip = "off") +
    
  # Labs
  labs(
    title = "Total Emissions Over Time for China (Coal) by Commodity",
    x = "Year",
    y = "Total Emissions (MtCO2e)"
  ) +
...
combined_plot <- (p1 | p2) +
    ...
    # Labs
    plot_annotation(
        title = "Analyzing Emissions Trends: A Focus on China (Coal)",
        subtitle = "An exploration of the major contributors to global emissions and a detailed look at China (Coal)",
