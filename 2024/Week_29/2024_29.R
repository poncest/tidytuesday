...
## Challenge: #TidyTuesday 2024 week 29
## Data:      English Women's Football
## Author:    Steven Ponce
## Date:      2024-07-17
...
tt <- tidytuesdayR::tt_load(
  ...
  week = base::as.double("29")
)

ewf_appearances <- tt$ewf_appearances |> clean_names() |> glimpse()
# ewf_matches     <- tt$ewf_matches |> clean_names() |> glimpse()
# ewf_standings   <- tt$ewf_standings |> clean_names() |> glimpse()
...
skim(ewf_appearances)


## 4. TIDYDATA ----

data <- ewf_appearances |>
...
### |-  titles and caption ----
# icons
tt <- str_glue("#TidyTuesday: { 2024 } Week { 29 } &bull; Source: The English Women's Football (EWF) Database, May 2024<br>")
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Team Performance Across English Women's Football Seasons")

subtitle_text <- str_glue("Analyzing wins, draws, and losses from 2011 to 2023")
...
### |-  final plot ----  
ggplot(data, aes(x = team_name, y = perc, fill = result)) +
    
    # Geoms
    geom_col(show.legend = FALSE) +
    geom_hline(yintercept = 0, color = 'gray40', linewidth = 0.15) +
    ...
    scale_fill_manual(values = c("Win" = "#377eb8", "Draw" = "#4daf4a", "Loss" = "#ff7f00")) +
    coord_flip(clip = 'off') +
