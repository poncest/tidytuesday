...
## Challenge: #TidyTuesday 2023 week 21
## Data:      Central Park Squirrels
## Author:    Steven Ponce
## Date:      2023-05-22
...
## 2. READ IN THE DATA ----
tt <- tidytuesdayR::tt_load(2023, week = 21) 

squirrel_data <- tt$squirrel_data %>% clean_names()
...
squirrel_tbl <- squirrel_data %>% 
    ...
    group_by(wday, primary_fur_color, running, chasing, eating, foraging) %>% 
    summarise(
        Running  = sum(running),
        Chasing  = sum(chasing),
        Eating   = sum(eating),
    ...
    # select columns
    select(wday:primary_fur_color, Running:Foraging) %>% 
    # long table
    pivot_longer(!c(wday, primary_fur_color), names_to = "activities", values_to = "count") %>% 
    # Make weekend the last two days
    mutate(wday = fct_relevel(wday, "Sun", "Sat", "Fri", "Thu", "Wed", "Tue", "Mon"))
...
### |-  titles and caption ----
tt <- str_glue("#TidyTuesday: 2023 Week 21 &bull; Source: 2018 Central Park Squirrel Census<br>")  
tw <- str_glue("<span style='font-family:fa6-brands'>&#xf099;</span>")
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

title_text    <- str_glue("Central Park Squirrels, October 2018") 
...
squirrel_tbl %>% 
    ...
    ggplot(aes(x = count, y = as_factor(wday) ,  fill = as_factor(primary_fur_color))) +
    
    # geoms
    geom_beeswarm(shape = 22, color = bkg_col , method = "swarm", cex = 1.1,
                  priority = "density", size = 2.5) +
    ...
    # labs
    labs(x = "Count (log10)", y = "",
         title    = title_text,
         caption  = caption_text) +
    
    # facets
    facet_wrap(~ activities, nrow = 4)+
    ...
    theme(
        ...
        axis.title         = element_text(size = 8, face = 'bold', color = text_col, margin = margin (r = 10)),  
        
        axis.ticks.x       = element_line(color = "white"),
        axis.line          = element_line(linewidth = 0.8, colour = "grey80"),
        ...
        strip.text         = element_textbox(size      = 9,
                                              face     = 'bold',
                                              color    = text_col,
                                              hjust    = 0.5,
                                              halign   = 0.5,
